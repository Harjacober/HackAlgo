...
change_authuser_pswd_parser.add_argument("pswd",help="user new password",required=True)
...
class AdminRegistration(Resource):
    ...
    def post(self):
        ...
        Timer(0.5,retry,(5,current_app.mail.send,msg),kw).start()
...
class ChangePassword(Resource):
    ...
    def  post(self):
        ...
        if hmac.compare_digest(current_app.pendindmacs.get(data["email"])[0],"")==id:
...
def ValidatePassword():
    ...
    ok = ok and hmac.compare_digest(current_app.pendindmacs.get(email)[0],"")==id
    if not ok:
